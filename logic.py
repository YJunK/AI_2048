...
def start_game():
	...
	add_new(mat)
	add_new(mat)										# added a exstra call of the add_new to spawn two numbers at the start of the game
...
def add_new(mat):										# changed from add_new_2(mat) to add_new(mat)
...
# row and column.
	...
	while(mat[r][c] != 0): 								# changed from mat[r] to mat[r][c]
		r = random.randint(0, 3)
		c = random.randint(0, 3)
	
	# we will place a 2 at that empty
	# random cell.
	draw = random.choices([2, 4], [0.9, 0.1])[0]		# added to ensure 90% of the time 2 is drawn and 10% of the time 4 is drawn
	mat[r][c] = draw									# changed from mat[r] = 2 to mat[r][c] = draw
